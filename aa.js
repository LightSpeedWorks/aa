...
this.aa = (function (PromiseThunk) {
  ...
  var setValue = defProp
    ? function setValue(obj, prop, val) {
        defProp(obj, prop, { value: val, writable: true, configurable: true });
      }
    : function setValue(obj, prop, val) {
        obj[prop] = val;
      };
  ...
  Queue.prototype.push = function push(x) {
    if (this.tail) this.tail = this.tail[1] = [x, null];
    else this.tail = this.head = [x, null];
  };
  ...
  function aa(gtor) {
    ...
    if (isPromise(gtor)) return PromiseThunk.resolve(gtor);

    // is function? then promisify it.
    if (typeof gtor === "function") return promisify.call(ctx, gtor);

    // is not generator?
    if (!isGenerator(gtor)) return Channel.apply(ctx, arguments);

    var resolve,
      reject,
      p = new PromiseThunk(function (res, rej) {
        resolve = res;
        reject = rej;
      });
    ...
    function callback(err, val) {
      try {
        if (err) {
          if (typeof gtor["throw"] !== "function") return reject(err);
  ...
  function doValue(value, callback, ctx, args) {
    ...
    if (isGeneratorFunction(value)) value = value.apply(ctx, args);

    if (isGenerator(value)) return aa.call(ctx, value)(callback);
      ...
      value.forEach(function (val, i) {
        doValue(val, function (err, val) {
          if (err) {
            ...
            arr[i] = val;
            if (--n === 0 && !called) (called = true), callback(null, arr);
          }
        });
      });
    } // array
      ...
      keys.forEach(function (key) {
        ...
        doValue(value[key], function (err, val) {
          if (err) {
            ...
            obj[key] = val;
            if (--n === 0 && !called) (called = true), callback(null, obj);
          }
        });
      });
    } // object
  ...
  if (setProto) setProto(Channel.prototype, Function.prototype);
  else {
  ...
  function Channel(empty) {
    if (arguments.length > 1) throw new Error("Channel: too many arguments");
    ...
    if (setProto) setProto(channel, Channel.prototype);
    else {
      channel.close = $$close;
      channel.done = $$done;
      channel.send = $$send;
    ...
    function channel(a, b) {
      // yield callback
      if (typeof a === "function") return channel.recv(a);

      // error
      if (a instanceof Error) return channel.send(a);

      // value or undefined
      if (arguments.length <= 1) return channel.send(a);

      var args = slice.call(arguments);

      if (a == null) {
        if (arguments.length === 2) return channel.send(b);
        else args.shift();
      }
  ...
  function done() {
    if (!this.$isDone && this.$isClosed && this.$sendValues.length === 0) {
      ...
      this.$recvCallbacks.forEach(function (cb) {
        complete(cb, empty);
      });
    }
  ...
  function complete(cb, val) {
    if (val instanceof Error) cb(val);
    else cb(null, val);
  } // complete
  ...
  if (GeneratorFunction) aa.GeneratorFunction = GeneratorFunction;
  ...
  if (Object.getOwnPropertyNames)
    Object.getOwnPropertyNames(PromiseThunk).forEach(function (prop) {
      if (!aa.hasOwnProperty(prop)) setValue(aa, prop, PromiseThunk[prop]);
    });
  else
